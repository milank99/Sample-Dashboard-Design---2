...
function createUtilityCard(utility) {
  ...
  const tagClass = isAI ? 'bg-violet-100 text-indigo-700' : 'bg-teal-100 text-teal-700';
  const iconName = isAI ? 'brain' : 'network';
  const iconColor = isAI ? 'text-indigo-600' : 'text-teal-600';

  return `
    <a href="${utility.url}" target="_blank" rel="noopener noreferrer"
       class="group block p-6 bg-white border-2 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${accentColorClass}">
      <div class="flex items-start justify-between">
        <div class="p-3 rounded-xl ${iconBgClass}">
          <i data-lucide="${iconName}" class="w-6 h-6 ${iconColor}"></i>
        </div>
        <i data-lucide="arrow-up-right" class="w-5 h-5 text-slate-300 group-hover:text-slate-600 transition-colors"></i>
      </div>
      <div class="mt-5">
      ...
      <div class="mt-4 flex items-center gap-2">
        <span class="text-[10px] font-bold tracking-wider uppercase px-2 py-1 rounded-md ${tagClass}">
          ${utility.type}
        </span>
      </div>
    </a>
